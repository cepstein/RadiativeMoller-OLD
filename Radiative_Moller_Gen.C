...
double te(double x) {return -4.*Ecmp*Ecmp*pow(sin(x/2.),2.);}

//Mandelstam U
//Double_t ue(Double_t x) {return 4.0*pow(me,2) - sv - tv(x);}
double ue(double x) {return -4.*Ecmp*Ecmp*pow(cos(x/2.),2.);}
...
double M1u (double x){return 2.*se/ue(x);}
    
double M2u (double x){return 2.*te(x)/ue(x);}

double M1t (double x){return 2.*se/te(x);}

double M3t (double x){return 2.*ue(x)/te(x);}


double M1uzp (double x){return gzp*gzp*2.*se/(ue(x)-mz2);}

double M1uzm (double x){return gzm*gzm*2.*se/(ue(x)-mz2);}

double M2uz (double x){return gzp*gzm*2.*te(x)/(ue(x)-mz2);}

double M1tzp (double x){return gzp*gzp*2.*se/(te(x)-mz2);}

double M1tzm (double x){return gzm*gzm*2.*se/(te(x)-mz2);}

double M3tz (double x){return gzp*gzm*2.*ue(x)/(te(x)-mz2);}
...
double Eq0(double Ek) {return 0.5*sqrt(Ecm*(Ecm-2.*Ek));}
double q0(double Ek){return sqrt(pow(Eq0(Ek),2.)-pow(me,2.));}

double betaq(double Ek){return Ek/(Ecm-Ek);}
double gammaq(double Ek){return pow(1.-betaq(Ek)*betaq(Ek),-0.5);}
...
// 
    ...
    for (int i = 0;i<ekDist->GetNbinsX();i++){ekDist->SetBinContent(i,1./(i+1.));}
    ...
    for (int i = 0;i<tkDist->GetNbinsX();i++){tkDist->SetBinContent(i,1./(sqrt(pow(pi/2.,2.)-pow((i/1000.)*pi-pi/2.,2.))));}
    tkDist->Scale(1./tkDist->Integral("width"));

    for (int i = 0;i<tqrDist->GetNbinsX();i++){tqrDist->SetBinContent(i,1./(sqrt(pow(pi/2.,2.)-pow((i/1000.)*pi-pi/2.,2.))));}
...
//            cout<<"k E "<<k->E()<<endl;
            ...
            cout<<"weight: "<<weight<<" cs: "<<dSigmahdEkdTkdTqr(Ek,tk,tqr,pqr)<<" Ek: "<<Ek<<" tk: "<<tk<<" tqr: "<<tqr<<" pqr: "<<pqr<<endl;
                ...
                if (q1->Theta() > tmin ){ if(q1->Theta() < tmax){if(q1->E() > Ecut){
            ...
            if (q2->Theta() > tmin ){ if(q2->Theta() < tmax){if(q2->E() > Ecut){
